...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<num_cams; ++i)
    {
        if(cams_worldpointDepths[i].size() > 0)
        {
            ...
            if(use_full_image_path)
                image = cv::imread(inputFolder+"/"+fname,cv::IMREAD_GRAYSCALE);
            else
                image = cv::imread(inputFolder+"/"+img_path.filename().string(),cv::IMREAD_GRAYSCALE);
    ...
    Line3D->saveSampledPoints(outputFolder);
